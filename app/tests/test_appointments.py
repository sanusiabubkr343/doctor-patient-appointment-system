...
def test_deny_overlapping_time_slot_creation(client, mock_authenticated_user, db):
    ...
    client.headers.update({"auth-header": f"Bearer {token}"})
    # Attempt to create an overlapping time slot
...
def test_time_slot_creation_with_invalid_dates(client, mock_authenticated_user):
...
def test_get_time_slot_by_id(client, mock_authenticated_user, db):
...
def test_delete_time_slot(client, mock_authenticated_user, db):
    ...
    # Verify that the time slot has been deleted
    response = client.get(f"/appointments/get-time-slot/{time_slot.id}")
    assert response.status_code == 404


def test_update_time_slot(client, mock_authenticated_user, db):
...
def test_get_all_time_slots(client, mock_authenticated_user, db):
...
def test_book_appointment(client, mock_authenticated_user, db, role, expected_status):
...
def test_complete_appointment(client, mock_authenticated_user, db, role, expected_status):
...
def test_cancel_appointment(client, mock_authenticated_user, db, initial_status, expected_status):
...
def test_get_all_appointments(client, mock_authenticated_user, db):
...
def test_get_appointment_by_id(client, mock_authenticated_user, db):
